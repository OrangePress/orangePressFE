...
function renderPackagesList() {
  ...
  container.innerHTML = sorted
    .map((pkg) => {
      ...
      const exemplarLine =
        pkg.benefits.find((b) => b.includes("Eksemplar")) || "";
      const metaParts = [exemplarLine, !isActive ? "Nonaktif" : ""]
        .filter(Boolean)
        .join(" • ");
  ...
  if (result.isConfirmed) {
    try {
      const res = await fetch(
        `https://orange-press-be.vercel.app/api/packages/${id}`,
        {
          method: "DELETE",
          headers: { Authorization: `Bearer ${token}` },
        },
...
function openEditPackage(id) {
  ...
  document.getElementById("editPackageBenefits").value =
    pkg.benefits.join("\n");
...
async function submitEditPackage(e) {
  ...
  const title = document.getElementById("editPackageTitle").value.trim();
  ...
  const benefits = document
    .getElementById("editPackageBenefits")
    .value.split("\n")
    .map((b) => b.trim())
    .filter(Boolean);

  if (!title || !price || !description || benefits.length === 0) {
    Swal.fire({
      icon: "warning",
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  // Modal controls
  document
    .getElementById("closeAddPackageModal")
    ?.addEventListener("click", closeAddPackageModal);
  document
    .getElementById("cancelAddPackage")
    ?.addEventListener("click", closeAddPackageModal);
  document
    .getElementById("addPackageForm")
    ?.addEventListener("submit", submitAddPackage);
...
document
  .getElementById("closeEditPackageModal")
...
document
  ...
  ?.addEventListener("submit", submitEditPackage);

document.getElementById("editPackageModal")?.addEventListener("click", (e) => {
  if (e.target.id === "editPackageModal") {
    closeEditPackageModal();
  }
});
